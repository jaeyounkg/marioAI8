...
public class LearningWithGA implements LearningAgent {
	...
	/*
	 * 1世代で生成したすべての個体を1回ずつステージでプレイさせ評価値を算出． その結果を降順にソート．もし過去の最高評価額(fmax)を超える個体が生成
	...
	/*
	 * GA のアルゴリズムにおける優良な遺伝子の選択行為を行っている。 これは交叉の親となる 2 つの遺伝子をルーレット戦略によって 選択するメソッドになる。
	 */
	private int[] select() {
		...
		for (int i = 0; i < 2; i++) { // 2回繰り返す
			...
			for (int j = bestnum + 1; j < popsize; j++) {
				...
				if (accumulateRoulette[j - 1] < selectedParent && selectedParent < accumulateRoulette[j]) {
					parentsGene[i] = j;
					break;
				}
